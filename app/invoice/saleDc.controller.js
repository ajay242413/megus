(function(){
    'use strict';
    angular.module('app').controller('SaleDc.IndexController', saleDcController);

    function saleDcController(SettingService, InventoryService, $state, $scope, $uibModal, DTOptionsBuilder, DTColumnDefBuilder){
        ...
        function initController(){
        	...
        	InventoryService.readInventory({type: 'sdc'}).then(function(res){
                $scope.result = res.data;
            });
        }

        $scope.newSaleDc = function(){
        	$state.go('invoice.addSaleDc');
        }
        ...
        $scope.moveToBill = function(res){
            if(res.totalQuantity != 0){
                $state.go('invoice.dcSaleBill', {obj: res});
            } else{
