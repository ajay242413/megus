(function() {
    'use strict';
    angular.module('app').controller('RdsSecondaryReport.IndexController', rdsSecondaryReportController);

    function rdsSecondaryReportController(MasterService, UserService, ClientService,  InventoryService, ExcelService, $scope, $state){
        ...
        $scope.generalOptChange = function(){
            switch($scope.generalOption){
                ...
                //             break;
                default: $state.go('reports.rdsSecondaryReport');
                            break;
            }
        }
        ...
        $scope.getReport = function(){
            if((angular.element($('#startFrom')).val()) && (angular.element($('#endAt')).val()) && ($scope.itemList.length)){
                ...
                InventoryService.rdsSecondarySummary({start:angular.element($('#startFrom')).val(), end:angular.element($('#endAt')).val(), item:$scope.itemList, imei:$scope.viewType}).then(function(res){
                    if(res.data.length){
        ...
        function genetrateJsonWithImei(result){
            ...
            for(var rdsInd = 0; rdsInd < $scope.rds.length; rdsInd++){
                if($scope.rds[rdsInd]['refID']['areaDistribution']){
                    ...
                    for(var dealerInd = 0; dealerInd < $scope.dealers.length; dealerInd++){
                        ...
                        if(check >= 0){
                            var catName = "";
                            if($scope.dealers[dealerInd]['category']){
                                catName = $scope.dealers[dealerInd]['category']['name']
                            }
                                        ...
                                        for(var imeInd1 = 0; imeInd1 < result[resultInd]['IMEI'].length; imeInd1++){
                                            for(var imeInd2 = 0; imeInd2 < result[resultInd]['IMEI'][imeInd1].length; imeInd2++){
                                                reportJson.push({
                                                    ...
                                                    'Dealer Name': $scope.dealers[dealerInd]['name'],
                                                    'Dealer Category': catName,
                                                    'Dealer Area': $scope.dealers[dealerInd]['area']['name'],
            ...
            ExcelService.exportCSV(reportJson, 'Date:,' + angular.element($('#startFrom')).val() + ',' + angular.element($('#endAt')).val(), 'RDS Secondary Report');
            swal.close();
        }
                ...
                if($scope.rds[rdsInd]['refID']['areaDistribution']){
                    ...
                    for(var dealerInd = 0; dealerInd < $scope.dealers.length; dealerInd++){
                        ...
                        if(check >= 0){
                            ...
                            if($scope.dealers[dealerInd]['category']){
                                reportJson[curInd]['Dealer Category'] = $scope.dealers[dealerInd]['category']['name']
                            }
                            for(var itemInd = 0; itemInd < $scope.itemList.length; itemInd++){
                                var resultInd = result.findIndex(x => (x._id.model === $scope.itemList[itemInd] && x._id.authorID === $scope.rds[rdsInd]['_id'] && x._id.party === $scope.dealers[dealerInd]._id));
                                if(resultInd >= 0){
