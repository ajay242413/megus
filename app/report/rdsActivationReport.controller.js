(function() {
    'use strict';
    angular.module('app').controller('RdsActivationReport.IndexController', rdsActivationReportController);

    function rdsActivationReportController(MasterService, UserService, InventoryService, ClientService, ExcelService, $scope, $state){
        ...
        function loadDefault(){
            ...
            $scope.viewType = 'type1';
            $scope.generalOption = 'Activation';
            refreshPicker();
            initController();
        }
        ...
        $scope.generalOptChange = function(){
            switch($scope.generalOption){
                ...
                //             break;
                default: $state.go('reports.rdsActivationReport');
                            break;
            }
        }
        ...
        $scope.getReport = function(){
            if((angular.element($('#startFrom')).val()) && (angular.element($('#endAt')).val()) && ($scope.itemList.length)){
                ...
                InventoryService.rdsActivationReport({start:angular.element($('#startFrom')).val(), end:angular.element($('#endAt')).val(), item:$scope.itemList, type:$scope.viewType}).then(function(res){
                    if(res.data.length){
        ...
        function genetrateJsonWithImei(result){
                                ...
                                if(resultInd >= 0){
                                    ...
                                    if(result[resultInd]['IMEI']){
                                        for(var imeInd1 = 0; imeInd1 < result[resultInd]['IMEI'].length; imeInd1++){
                                            ...
                                            if(result[resultInd]['IMEI'][imeInd1]['info']){
                                                if(result[resultInd]['IMEI'][imeInd1]['info']['date']){
                                                    reportJson[reportJson.length - 1]['Activation Date'] =  result[resultInd]['IMEI'][imeInd1]['info']['date'];
                                                }
                                                if(result[resultInd]['IMEI'][imeInd1]['info']['circle']){
                                                    reportJson[reportJson.length - 1]['Circle'] =  result[resultInd]['IMEI'][imeInd1]['info']['circle'];
                                                }
                                                if(result[resultInd]['IMEI'][imeInd1]['info']['operator']){
                                                    reportJson[reportJson.length - 1]['Operator'] =  result[resultInd]['IMEI'][imeInd1]['info']['operator'];
                                                }
                                            }
                                        }
                                    } else{
            ...
            ExcelService.exportCSV(reportJson, 'Date:,' + angular.element($('#startFrom')).val() + ',' + angular.element($('#endAt')).val(), 'RDS Activation Report');
            swal.close();
        }
